...
export const useProjectData = (category: ProjectCategory) => {
  return useQuery({
    queryKey: ['projects', category],
    queryFn: () => getProjectsByCategory(category),
    initialData: () => getProjectsByCategory(category)
  });
};

//Removed useAllProjectData hook as it's no longer needed with the new implementation.  A new implementation would need to be created if this functionality is still required, using the new useProjectData hook.

//The following code is removed as it is no longer necessary with the new data loading method.
//import { useQuery } from '@tanstack/react-query';
